...
<script>
    ...
    function showUpdate(obj){
        ...
        editor.customConfig.uploadImgServer = '/admin/book/upload' //设置图片上传路径
		...
		//发送ajax请求,获取对应图书信息
        $.get("/admin/book/id/" + bookId , {} , function(json){
			//文本框回填已有数据
    ...
    function showCreate(){
        ...
        editor.customConfig.uploadImgServer = '/admin/book/upload';//设置图片上传地址
    ...
    layui.form.on('submit(btnSubmit)', function(data){
		...
		//向服务器发送请求
        $.post("/admin/book/" + optype , formData , function(json){
            if(json.code=="0"){
    ...
    function showDelete(obj){
        ...
        layui.layer.confirm('确定要执行删除操作吗?', {icon: 3, title:'提示'}, function(index){
				...
				$.get("/admin/book/delete/" + bookId, {}, function (json) {
					if(json.code=="0"){
						//删除成功刷新表格
						table.reload('bookList');
						//提示操作成功
