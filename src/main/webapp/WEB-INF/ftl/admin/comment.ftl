...
<script>
    ...
    table.render({
        ...
        , limit :20
        , cols: [[ //表头
            {field:"createTime" , title: '发布时间', width: '200',templet : "<div>{{layui.util.toDateString(d.ordertime, 'yyyy年MM月dd日 HH:mm:ss')}}</div>"}
            ...
            {type: "space" , title: '禁用原因', width: '100', templet: function (d) {
                    console.info(d);
                    return d.disable_reason;
                }}
            ,
